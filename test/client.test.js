...
test('accepts a pre-existing ioredis connection', (t) => {
  ...
  client.set('foo', 'foo', 1000)
    .then(() => client.has('foo'))
    .then((result) => t.equal(result, true))
    .catch(t.threw)
})
...
test('set() sets an item', (t) => {
  ...
  client.set('foo', 'foo', 10000)
    ...
    .then((result) => {
      ...
      t.ok(result.item)
      t.equal(result.item, 'foo')
      t.equal(result.ttl > 0, true)
    })
    .catch(t.threw)
})
...
test('get() returns `null`', (t) => {
  ...
  const client = factory({ client: new MockIoredis() })
  client.get('foo')
    .then((result) => t.equal(result, null))
    .catch(t.threw)
})
...
test('scan() returns an array when only cursor is provided', (t) => {
  ...
  client.set('foo', 'foo', 1000)
    ...
    .then((result) => {
      t.type(result, Array)
      t.type(result[0], 'string')
      t.type(result[1], Array)
      t.equal(result[1][0], 'foo')
    })
    .catch(t.threw)
})
